...
<template>
    ...
    <app-progress v-if="isLoading" class="flex align-items-center h-full" />
    <app-message v-else-if="!isLoading && !interviews.length" severity="info">Нет добавленных собеседований</app-message>
    <div v-else>
        <h1>Список собеседований</h1>
        <div class="inline-flex align-items-center flex-wrap gap-2 p-2 mb-5 border-1 border-solid border-black-alpha-10 border-round-sm">
        ...
        <app-datatable :value="interviews">
            ...
            <app-column header="Результат">
                <template #body="slotProps">
                    ...
                    <template v-else>
                        <app-badge :severity="slotProps.data.result === 'Offer' ? 'success' : 'danger'">{{ slotProps.data.result === 'Offer' ? 'Оффер' : 'Отказ' }}</app-badge>
                    </template>
                </template>
            </app-column>
            <app-column>
